...
class BaseClient(object):
    def __init__(self, address='http://127.0.0.1:8545', async=True, async_timeout=10):
        self.is_async = async
    ...
    def _make_request(self, method, params):
        payload = self.construct_json_request(method, params)
        response = self.session.post( self.address, data=payload, headers=self.headers )
...
class JSONRPCBaseClient(BaseClient):
    ...
    def get_json_request(self, method, params):
        return self.construct_json_request( method, params )
    ...
    ####################### PERSONAL APIs should be enabled on http ######################
    # --rpcapi "web3,personal"
    def create_newAccount(self, password):
        """
        https://github.com/ethereum/go-ethereum/wiki/Management-APIs#personal_newaccount
        """
        response = self.make_request("personal_newAccount", [password] )
        return response['result']

    def unlock_account(self, address, password, duration=0 ): # we need it to remain unlocked for this session
        """
        https://github.com/ethereum/go-ethereum/wiki/Management-APIs#personal_unlockaccount
        """
        response = self.make_request("personal_unlockAccount", [str(address),str(password),int(duration)] )
        return response['result']

    def lock_account(self, address ): # extra
        """
        https://github.com/ethereum/go-ethereum/wiki/Management-APIs#personal_lockAccount
        """
        response = self.make_request("personal_lockAccount", [str(address)] )
        return response['result']
    ##################################################################
    def get_privatekey(self, address, password ):
        """
        hardcode :get key from 'eth' node
        """
        ...
        fileNamePattern = "--"+address[2:]
        fileName=''
        for file in out1.split():
          if file.find(fileNamePattern) > 0:
            fileName = file
        # get file content
        ...
        response = self.make_request( "personal_importRawKey", [str(address),str(password)] )
        return response['result']
